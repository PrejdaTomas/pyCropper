...
if runConstants.RUNMODE== RunMode.TEST:
	...
	from A_DEPENDENCIES import aPoint, aColour, aCorners, ImageOrientation, math
...
def getRatio(x0: int, y0: int, x1: int, y1: int) -> typing.Tuple[int,int]:
	width	= x1 - x0
	height	= y1 - y0
	commonDemonimator = math.gcd(width, height)
	if commonDemonimator == 0:
		return width,height
	width	= width // commonDemonimator
	height	= height // commonDemonimator
	return width, height

def getWidthHeightFromAR(width: Number, height: Number, aspectRatio: Number) -> typing.Tuple[float, float]:
	# print(f"startwidth={width}, startheight={height}")
	...
	# print(f"endwidth={width}, endheight={height}\n")
	return width, height
...
class Point(object):
	...
	def fromAbsCoords(cls, x_abs: int, y_abs: int, img: cv2t.MatLike) -> Point:
		xRel = x_abs/img.shape[1]
		yRel = y_abs/img.shape[0]
	...
	def moveByAspectRatio(self, start: Point, aspectRatio: float) -> Point:
		...
		print(aspectRatio, (start.x_rel, self.x_rel, width, nuXrel), (start.y_rel, self.y_rel, height, nuYrel))
...
def greenInfo(imageOrientation: ImageOrientation, disp: cv2t.MatLike, corners: Rectangle) -> None:
	...
	E_FUNC_UTIL.greenRectangle(imageOrientation, disp, abs_pt0, abs_pt1)
	E_FUNC_UTIL.greenText(imageOrientation, disp, abs_pt0, f"D{pt0}")
	E_FUNC_UTIL.greenText(imageOrientation, disp, abs_c, f"{width}:{height}")
	E_FUNC_UTIL.greenText(imageOrientation, disp, abs_pt1, f"D{pt1}")
...
def redWarning(imageOrientation: ImageOrientation, disp: cv2t.MatLike, corners: Rectangle) -> None:
	...
	E_FUNC_UTIL.redRectangle(imageOrientation, disp, abs_pt0, abs_pt1)
	E_FUNC_UTIL.redText(imageOrientation, disp, abs_pt0, f"D{pt0}")
	E_FUNC_UTIL.redText(imageOrientation, disp, abs_c, f"{width}:{height}")
	E_FUNC_UTIL.redText(imageOrientation, disp, abs_pt1, f"D{pt1}")
...
def drawArrow(disp: cv2t.MatLike, corners: Rectangle, colour: aColour, thickness: int) -> None:
	...
	abs_pt0	= pt0.toAbsCoords(disp)
	abs_pt1	= pt1.toAbsCoords(disp)
	E_FUNC_UTIL.arrow( disp, abs_pt0, abs_pt1, colour, thickness)
