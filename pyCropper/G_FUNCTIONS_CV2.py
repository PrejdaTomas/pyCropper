...
from .A_DEPENDENCIES import aPoint, aCorners, aColour, Path, CV2_ESCAPE, CV2_KEYS
...
greenRectangle:		typing.Callable[[cv2.typing.MatLike, aPoint, aPoint], cv2.typing.MatLike]	= lambda DISP, PT1, PT2: cv2.rectangle(DISP, PT1, PT2, green(), 2)
redRectangle:		typing.Callable[[cv2.typing.MatLike, aPoint, aPoint], cv2.typing.MatLike]	= lambda DISP, PT1, PT2: cv2.rectangle(DISP, PT1, PT2, red(), 2)
redText:			typing.Callable[[cv2.typing.MatLike, aPoint, str], cv2.typing.MatLike]		= lambda DISP, PT, text: cv2.putText(DISP, text, PT, cv2.FONT_HERSHEY_COMPLEX, 0.5, red())
greenText:			typing.Callable[[cv2.typing.MatLike, aPoint, str], cv2.typing.MatLike]		= lambda DISP, PT, text: cv2.putText(DISP, text, PT, cv2.FONT_HERSHEY_COMPLEX, 0.5, green())



arrow: typing.Callable[[cv2.typing.MatLike, aPoint, aPoint, aColour], cv2.typing.MatLike]= lambda DISP, PT1, PT2, COLOR=(0,0,0),THICKNESS=1: cv2.arrowedLine(	DISP, PT1,PT2,COLOR,THICKNESS)
...
def greenInfo(disp: cv2.typing.MatLike, corners: aCorners) -> None:
	pt0, pt1 = corners
...
def redWarning(disp: cv2.typing.MatLike, corners: aCorners) -> None:
	pt0, pt1 = corners
...
printImageStatus: typing.Callable[[	str,
									...
									typing.Tuple[int,int]], None
																								...
																								sep=", ")
def getCroppedCV2Img(	source: cv2.typing.MatLike,
						corners: aCorners = controlConstants.RECT_CORNERS) -> typing.Optional[cv2.typing.MatLike]:
	if len(corners) == 2:
		x0, y0 = corners[0]
		x1, y1 = corners[1]
...
def applyMask(img: cv2.typing.MatLike, start: aPoint, end: aPoint, alpha=0.6) -> cv2.typing.MatLike:
...
def processImage(imagePath: str, fileType=C_CLASSES.ImageType.JPG) -> cv2.typing.MatLike:
	#region declarations
	imageName: str
	copyName: str
	copyAddress: str
	...
	clone: cv2.typing.MatLike
	landscape: cv2.typing.MatLike
	portrait: cv2.typing.MatLike
	display: cv2.typing.MatLike
	corners: aCorners
	...
	if not controlConstants.SKIP:
		...
		if croppedClone is not None:
			if		fileType == C_CLASSES.ImageType.JPG:	cv2.imwrite(filename=copyAddress, img=croppedClone, params=[int(cv2.IMWRITE_JPEG_QUALITY), userConstants.JPG_QUALITY])
			elif	fileType == C_CLASSES.ImageType.PNG:	cv2.imwrite(filename=copyAddress, img=croppedClone, params=[int(cv2.IMWRITE_PNG_COMPRESSION), userConstants.PNG_QUALITY])
...
def processImages(	workDir: typing.Optional[Path] = userConstants.FROM_FOLDER,
					outputImageType: typing.Optional[C_CLASSES.ImageType] = C_CLASSES.ImageType.JPG
