...
if __name__ == "__main__":
	...
	from A_DEPENDENCIES import os, cv2, typing, time
	...
	import E_FUNC_UTIL
	import F_GEOM
	from F_GEOM import Point, Rectangle
...
class ImageWrapper(Picklable, B_DESCRIPTORS.ImageWrapperTYPECHECK):
	...
	def reset(self) -> None:
		while self.rotationState != 0: self.rotateClockwise()
		if self.verticalState: self.flipVertical()
		if self.horizontalState: self.flipHorizontal()
...
class Displayer(Picklable):
			...
			try:
				...
				if self.croppingAllowed and not self.rectangle:
					AR = 		(self.targetAR[1]/self.targetAR[0]) \
						if		(self.handler.preview.imgOrigOrient == ImageOrientation.LANDSCAPE) \
						else	(self.targetAR[0]/self.targetAR[1])
					...
					print(cornersPreview.width, cornersPreview.height, cornersPreview)
					print()
				...
				if cornersPreview:
					if not self.rectangle:
						...
						F_GEOM.drawArrow(disp=self.handler.preview.image, corners=drawR, colour=(0,0,0,), thickness=4)

					if cornersPreview.largeEnough(self.handler.preview.image): 
						F_GEOM.greenInfo(	imageOrientation=self.handler.preview.imgOrigOrient,
											disp=self.handler.preview.image,
											corners=cornersPreview)

					else:
						F_GEOM.redWarning(	imageOrientation=self.handler.preview.imgOrigOrient,
											disp=self.handler.preview.image,
											corners=cornersPreview)
				...
				elif key in E_FUNC_UTIL.ords('t'):
	...
	def clickAndCrop(event: int, x: int, y: int, flags: int, param: Displayer) -> None:
		...
		if event == cv2.EVENT_LBUTTONDOWN:
			# print(f"click {self.clickState.value}, {self.targetAR}: {self.mousePos} -> {unscaledPoint}", self.handler.preview.shape)
			if self.clickState == ClickStates.NOTHING:
				...
				self.croppingAllowed	= False
				AR = self.targetAR[1]/self.targetAR[0]
				self.corner_2			= F_GEOM.clampPoint(unscaledPoint.moveByAspectRatio(	start=self.corner_1, aspectRatio=AR))
	...
	def storeImage(self, filePath: Path) -> None:
		#if not self.rectangle: raise IOError(f"{self}.storeCropped: impossible to store an image, the selection rectangle is undefined")
		#print(f"Cropping: starting @ {self.rectangleDiagonal}, preview = {self.handler.preview.diagonal}|{self.handler.preview.ARfloat}, image={self.handler.image.diagonal}|{self.handler.image.ARfloat}")
		if not self.rectangle:
...
class FolderProcessor(Picklable):
	...
	@property
	def sources(self) -> typing.List[Path]: return [Path(os.path.join(self.sourceFolder, imagePath)) for imagePath in os.listdir(self.sourceFolder) if E_FUNC_UTIL.getFileSuffix(imagePath).lower() in allowedFileTypes]
